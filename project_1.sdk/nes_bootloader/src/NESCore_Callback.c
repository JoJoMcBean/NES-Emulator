...
#define SNES_BASEADDR	0x41230000
...
void NESCore_Callback_InputPadState(dword *pdwPad1, dword *pdwPad2) {
	...
	byte j_up = (joy_state & 0x02) << 3; //10000
	byte j_down = (joy_state & 0x01) << 5; //100000
	byte j_left = (joy_state & 0x04) << 4; //1
	byte j_a = (joy_state & 0x40) >> 6; //1
	byte j_b = (joy_state & 0x80) >> 6; //10

	*pdwPad1 = 0;
	*pdwPad1 = j_up | j_down | j_left | j_right | j000000
	byte j_right = (joy_state & 0x08) << 4; //10000000_b | j_a | b_start | b_select;
	*pdwPad2 = 0;
#else
#ifdef SNES
	int snes_state = ~(*((unsigned int *) SNES_BASEADDR));

	byte snes_a = (snes_state & 0x100) >> 8;	//1
	byte snes_b = (snes_state & 0x1) << 1;		//10
	byte snes_sel = (snes_state & 0x4);
	byte snes_st = snes_state & 0x8;
	byte snes_dpad = snes_state & 0xf0;

	*pdwPad1 = snes_a | snes_b | snes_sel | snes_st | snes_dpad;
	*pdwPad2 = 0;
	...
	byte d_down = (dpad_state & 0x01) << 5;//100000
	byte d_left = (dpad_state & 0x04) << 4;//1000000
	byte d_right = (dpad_state & 0x08) << 4;//10000000
